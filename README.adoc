...
`interval` type. But let's say we wanted to create our own Python class to be sent as
an `interval` type. Then we'd have to register an adapter:
...
>>>
>>> con.register_py_to_db(MyInterval, my_interval_py_to_db)
...
type. But let's say we wanted to create our own Python class to be sent as an
`interval` type. Then we'd have to register an adapter:
...
* Upgrade the drivers to their latest versions.

* The `register_py_to_db` method now only takes two parameters.
